...
program main
    ...
    complex*16 :: eigenvalues(N), work(3*N-2), eigenvectors(N,N)
    ! allocating 
...
! open file to write
    ...
    ! Call LAPACK eigensolver
            call zheev('Vectors', 'U', N, ham_k, lda, eigenvalues, dummy,lwork, work, info)
            lwork = max((nb+1)*N, nint(real(dummy(1))))
            allocate(work(lwork))
            call zheev('Vectors', 'U', N, ham_k, lda, eigenvalues, work,lwork, work, info)
...
subroutine create_lattice(lattice,delta_l,reciprocal_lattice)
    ...
    reciprocal_lattice(:,1)= (/-1*lattice(2,1),lattice(1,1)/)
    reciprocal_lattice(:,2)= (/-1*lattice(2,2),lattice(1,2)/)
    return 
    END
